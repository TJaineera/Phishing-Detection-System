    ...
    try:
        ...
        with socket.create_connection((host, port)) as sock:
            with context.wrap_socket(sock, server_hostname=host) as ssock:
                ...
                # Print the SSL certificate details
                print("SSL Certificate Details:")
                print(f"Common Name (CN): {cert['subject'][0][0][1]}")
                print(f"Issuer: {cert['issuer'][0][0][1]}")
                print(f"Valid From: {cert['notBefore']}")
                print(f"Valid Until: {cert['notAfter']}")
                print(f"Subject Alternative Names (SANs): {cert.get('subjectAltName', '')}")
...
if __name__ == "__main__":
    ...
    user_url_for_ssl = input("Enter the URL to check SSL certificate: ")
    check_ssl_certificate(user_url_for_ssl)
