...
def check_phishing_indicators(url):
    ...
    if contains_ip_address(url):
        indicators.append("4. The URL contains an IP address instead of a domain name, which is uncommon for legitimate sites.")
...
def analyze_url(url):
    try:
        ...
        if html_content:
            if is_secure(url):
                secure_line = "The website appears to be secure (HTTPS).\n"
                output += secure_line
                print(secure_line)
                sg.popup(secure_line, title="Secure Website", line_width=60, text_color="green")

            output += "\nAdditional Details:\n"
            output += f"Content Type: {headers.get('content-type', 'N/A')}\n"
            output += f"Server: {headers.get('server', 'N/A')}\n"
            output += f"X-Powered-By: {headers.get('x-powered-by', 'N/A')}\n"


            if "The website appears to be secure" in output:
                print("\033[92m" + "The website appears to be secure (HTTPS).\n" + "\033[0m")
            else:
                print(output)

        else:
            message = "Unable to fetch HTML content for analysis. This could be a phishing attempt."
            output += message + "\n"
            sg.popup(message, title="Phishing Warning", line_width=60, text_color="red")
...
def check_ssl_certificate_gui(url):
    try:
        # Parse the URL to get host and port
        parsed_url = urlparse(url)
        host = parsed_url.hostname
        port = parsed_url.port or 443  # Default to 443 if port is not specified in the URL
        ...
        with socket.create_connection((host, port)) as sock:
            with context.wrap_socket(sock, server_hostname=host) as ssock:
                ...
                # Simplify the output
                result = {
                    "Common Name (CN)": cert["subject"][0][0][1],
        ...
        print("It is advisable to exercise caution when accessing this website; this might be phishy!")
        return f"Error checking SSL certificate: {e}"


def main():
    layout = [
        [sg.Text("Enter the URL to analyze: "), sg.InputText(key="url_input")],
        [sg.Button("Analyze URL"), sg.Button("Check SSL Certificate")],
        [sg.Text("Result:", size=(10, 1)), sg.Output(size=(80, 20), key="output")]
    ]

    window = sg.Window("URL Analyzer", layout)
    ...
    while True:
        ...
        if event == sg.WINDOW_CLOSED:
            break
        elif event == "Analyze URL":
            user_url = values["url_input"].strip()
            window["output"].update("")
            with redirect_stdout(window["output"]):
                analyze_url(user_url)
        elif event == "Check SSL Certificate":
            user_url_for_ssl = values["url_input"].strip()
            window["output"].update("")
            with redirect_stdout(window["output"]):
                result = check_ssl_certificate_gui(user_url_for_ssl)
